...
SELECT DISTINCT 
	  ...
	  CASE (LEFT(C.CL_IVA,20)) 
		...
		'2' AS [XX12REGIIBB],											--Régimen IIBB  1) CM,  2) Local,  3) Todas
...
/*ISNULL((SELECT STUFF ((SELECT ' (' + */(CASE CC.CLC_TIPO --FunciónA
	...
	OPEN CLIENTESSIGLA
		...
		BEGIN
			...
			SELECT distinct 
				...
				clienteTmp.CRY, clienteTmp.LAN,	 case (clienteTmp.CRN)  when '' then '30999999995' else clienteTmp.CRN end ,  clienteTmp.CUR,  clienteTmp.ABCCLS, clienteTmp.DME, clienteTmp.FREINV,  
				...
				clienteTmp.[BPAADD],  clienteTmp.[ACCCOD], clienteTmp.[VACBPR], clienteTmp.[PTE], case(clienteTmp.XX12NIF) when '' then '30999999995'else clienteTmp.XX12NIF end , clienteTmp.[XX12DOCTYP], 
				 clienteTmp.XX12DOCNUM , clienteTmp.[XX12REGIIBB], case (clienteTmp.XX12IIBB)when '' then '30999999995'else clienteTmp.XX12IIBB end , clienteTmp.[XX12SITGAN], 
	...
	,ISNULL(tmpData.Dato36,'') ,ISNULL(tmpData.Dato35,'') ,ISNULL(tmpData.Dato38,''),ISNULL(tmpData.Dato39,''),ISNULL(tmpData.Dato40,'')
