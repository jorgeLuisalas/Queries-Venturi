...
(
 ...
 select DISTINCT 
			...
			(SELECT CASE UPPER(LEFT (prov.PO_DESCRI,35))         --País
							--LISTADO PAISES
			...
			LEFT(P.PR_DIRECC,50)      as '[BPAADDLIG(1)]',   --Línea de dirección
			''                        as '[BPAADDLIG(2)]',   --Línea de dirección
			left(p.PR_CODPOS,10)      as 'POSCOD',           --Código postal */
         ...
         /* 'R'                  as 'INDICADOR_R',
			...
			/*'C'                 as 'INDICADOR_C',
			 RIGHT(100000 + Row_number () Over(order by (select p.PR_CODIGO)ASC),5) AS 'CCNCRM',                      --Código de contrato
...
if OBJECT_ID('tempdb..##TMPGRID') IS NOT NULL
	...
	DECLARE @descripción NVARCHAR(MAX)
	...
	OPEN TERCEROS
		FETCH TERCEROS INTO  @Codprov,@sigla,@descripción 
		...
		INSERT INTO ##TMPGRID
			SELECT distinct 
			      TT.INDICADOR_A,TC.Codprov,TT.BPRNAM,TC.Descripción,TT.BPRLOG,TT.CUR,TT.CRY,TT.LAN,TT.CRN,TT.XX12IIBB,TT.DOCTYP,TT.XX12DOCNUM,TT.XX12NIF
