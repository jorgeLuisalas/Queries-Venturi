...
INSERT INTO @TMPTABLE_ARTICULOS
	SELECT           
	   'I' AS 'INDICADOR_I',
		...
		LEFT('PT', 5) as TCLCOD,												  --Aqui hay que ver algunos números especificos, CATEGORIA DE LOS ARTICULOS	
		...
		*/
		UPPER(LEFT(REPLACE(REPLACE(RTRIM(A.[codArt]),'-', ''), '.', ''), 20)) AS [ITMREF],   --CÓDIGO DEL ARTICULO SIN ESPACIOS NI GUIONES
		UPPER(LEFT(A.[desc], 30)) AS DES1AXX,											  --Descripción 1er máximo 30 caracteres		
		'' AS DES2AXX,								      --Descripción 2do máximo 30 caracteres
		'' AS DES3AXX,									  --Descripción 3er máximo 30 caracteres
		'' AS SEAKEY,																	      --Clave de busqueda
		'' AS CFGLIN,																		  --Linea de producto		
		'' AS EANCOD,																  --Código EAN
		'' AS ITMSTD,																		  --Norma
		'' AS LIFSTRDAT,														              --Inicio de vida
		...
		*/																					  --Articulo reemplazo
		...
		CASE A.[Unidad]
			...
			WHEN ''   THEN 'UN'
 			ELSE 'UN'
		END  AS [PUU],														  --Unidad compra
		'1' AS PUUSTUCOE,																      --Coef. UC-US   1BAG = 10Unid
		CASE A.[Unidad]
			WHEN 'MT' THEN 'M'
			WHEN 'UN' THEN 'UN'
			WHEN 'KG' THEN 'KG'
			WHEN ''   THEN 'UN'
 			ELSE 'UN'
		END AS [SAU],														  --Unidad Venta
		'1' AS SAUSTUCOE,																	  --Coef. UV-US   1 BAG = 10 UN
		'' AS [PCU(0)],																		  --Envasado 0 y 1
		'' AS [PCUSTUCOE(0)],																  --Coef. UE-US (0) y (1)
		'' AS [PCU(1)],																		  --Envasado 0 y 1
		0 AS [PCUSTUCOE(1)],																  --Coef. UE-US
		'STD' AS [ACCCOD],																	  --Código contable (A CONSULTAR)
		'NOR' AS [VACITM(0)],																  --Nivel impuesto  (A CONSULTAR)
		...
		LEFT('', 15) AS [BPSNUM],													  --Proveedor
		'' AS [ITMREFBPS],										                              --Artículo proveedor
		...
		'UMTX'  AS [ODFCOD],																	   --Otros datos locales
		'1'  AS [ODFVAL],																	   --Valor
		...
		' ' AS [XX12CPY],															           --Sociedad
		' ' AS [XX12BASTYP1],														           --Tipo de carga
		' ' AS [XX12BASTYP2]														           --Actividad



	FROM [Varios].[dbo].[t_ArtDat] A
...
declare @TMPTABLE_ARTICULOSDATA table (	
   ...
   FamiliaEstadistica4 VARCHAR(10) NOT NULL
												...
												--	codArt		codProv	 fe1		fe2			fe3		   fe4		fe5
 ...
 CREATE TABLE ##TMP_GRID(
	...
	DECLARE ARTICULOSDATA CURSOR 
		...
		FOR
			SELECT codArt, CodProv, FamiliaEstadistica, FamiliaEstadistica1, FamiliaEstadistica2, FamiliaEstadistica3, FamiliaEstadistica4
		...
		OPEN ARTICULOSDATA
			...
			BEGIN
					...
					select distinct       
					'I', art.TCLCOD, art.[ITMREF], art.[DES1AXX], art.DES2AXX, art.DES3AXX, art.SEAKEY,	art.CFGLIN, art.EANCOD, 
					art.ITMSTD, art.LIFSTRDAT, art.LIFENDDAT, art.[TSICOD(0)], art.[TSICOD(1)], art.[TSICOD(2)], art.[RPLITM], art.[LOTMGTCOD], 
					art.[LOTCOU],art.EXYMGTCOD ,art.[SHL], art.EXYSTA, art.SHLLTI, art.NEWLTISTA, art.STU, art.[WEU], art.[ITMWEI], art.[PUU],
					...
					FROM @TMPTABLE_ARTICULOS art
					INNER JOIN [Varios].[dbo].[t_ArtDat] artd
					ON art.ITMREF = artd.codArt
					WHERE art.ITMREF = @Item

				
					INSERT INTO ##TMP_GRID
					SELECT DISTINCT
					'S', art.SBSITM, art.PCK, art.PCKCAP, art.BASPRI, art.[S*71], art.[S*72], art.[S*81], 
					art.[S*82], '', '', '', '', '', '', '', '', '', '', '', '',	'', '', '', '', '', '', '',	'',	'', '',	'', 
					'', '', '', '', '', '', '', '', '',	'', '',	'', '', '', ''
					FROM @TMPTABLE_ARTICULOS art
					INNER JOIN [Varios].[dbo].[t_ArtDat] artd
					...
					INSERT INTO ##TMP_GRID
					SELECT DISTINCT 
					art.INDICADOR_P,'', art.[ITMREFBPS], art.[PIO], art.[EANCODBPS], art.[PUU], 
					...
					INSERT INTO ##TMP_GRID
					SELECT DISTINCT 
					'Q', art.[XX12TYPBAS], art.[XX12CPY], art.[XX12BASTYP1], art.[XX12BASTYP2], '', '', '', '', '', '', '', '', '', '', '', '',  
					'', '', '', '', '', '', '', '' , '', '', '', '', '', '', '', '', '', '',  '', '', '',  '', '', '', '', '', '', '', '', ''
					FROM @TMPTABLE_ARTICULOS art
					INNER JOIN [Varios].[dbo].[t_ArtDat] artd
					ON art.ITMREF = artd.codArt
	...
	SELECT isnull(tmpGrid.Dato1,  ''), isnull(tmpGrid.Dato2, ''),  isnull(tmpGrid.Dato3, ''),  isnull(tmpGrid.Dato4, ''),  isnull(tmpGrid.Dato5, ''),
		   ...
		   isnull(tmpGrid.Dato46, ''),isnull(tmpGrid.Dato47, '')
